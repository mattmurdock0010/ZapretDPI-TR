...
WINWS2=${WINWS2:-${ZAPRET_BASE}/nfq2/winws2.exe}
MDIG=${MDIG:-${ZAPRET_BASE}/mdig/mdig.exe}
...
DNSCHECK_DOM=${DNSCHECK_DOM:-discord.com ej.ru rutracker.org www.torproject.org bbc.com}
...
curl_test_http()
{
	...
	curl_probe $1 "$2" $HTTP_PORT "$3" -SsD "$hdrt" -A "$USER_AGENT" --max-time $CURL_MAX_TIME $CURL_OPT "http://$2" -o NUL 2>&1 || {
		code=$?
		rm -f "$hdrt"
		return $code
	}
...
curl_test_https_tls12()
{
	...
	curl_probe $1 "$2" $HTTPS_PORT "$3" $HTTPS_HEAD -Ss -A "$USER_AGENT" --max-time $CURL_MAX_TIME $CURL_OPT --tlsv1.2 $TLSMAX12 "https://$2" -o NUL 2>&1
}
curl_test_https_tls13()
{
	...
	# force TLS1.3 mode
	curl_probe $1 "$2" $HTTPS_PORT "$3" $HTTPS_HEAD -Ss -A "$USER_AGENT" --max-time $CURL_MAX_TIME $CURL_OPT --tlsv1.3 $TLSMAX13 "https://$2" -o NUL 2>&1
}
...
curl_test_http3()
{
	...
	curl_with_dig $1 "$2" $QUIC_PORT $HTTPS_HEAD -Ss -A "$USER_AGENT" --max-time $CURL_MAX_TIME_QUIC --http3-only $CURL_OPT "https://$2" -o NUL 2>&1
}
...
pktws_start()
{
	case "$UNAME" in
		...
		CYGWIN)
			...
			cmd.exe /c "$(cygpath -w "$WINWS2")" --wf-dup-check=0 --wf-tcp-empty=1 $WF --ipset="$(cygpath -w "$IPSET_FILE")" \
				--lua-init="timer_set('exit_guard',function(name,data) io.stderr:write('exit_guard\n'); os.exit(3000); end,20000,true)" \
				--lua-init=@"$(cygpath -w "$ZAPRET_BASE/lua/zapret-lib.lua")" --lua-init=@"$(cygpath -w "$ZAPRET_BASE/lua/zapret-antidpi.lua")" "$@" >/dev/null &
			;;
	esac
	PID=$!
...
ws_kill()
{
	...
	[ -z "$PID" ] || {
		...
		[ "$UNAME" = "CYGWIN" ] && taskkill.exe /F /IM winws2.exe >/dev/null 2>&1
		PID=
	}
}
